...
#predict
pred_ckd <- predict(model,newdata = data_train, type = "class")
summary(pred_ckd)
...
rocrpred_gbm <- prediction(get_predict,data_train$CKD)
...
cutoffs <- data.frame(cut=rocrperf_gbm@alpha.values[[1]], fpr=rocrperf_gbm@x.values[[1]], 
...
table(Actualvalue = data_train$CKD, Predictedvalue = mod_predict_train>0.1986)
table(Actualvalue = data_train$CKD, Predictedvalue = mod_predict_train>0.17112)

table(Actualvalue = data_train$CKD, Predictedvalue = mod_predict_train>0.1154)
##########
